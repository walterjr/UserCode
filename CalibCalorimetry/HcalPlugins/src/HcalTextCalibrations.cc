  ...
  
{
  ...
  for (; request != data.end (); ++request) {
    ...
    else if (objectName == "QIEType") {
      setWhatProduced (this, &HcalTextCalibrations::produceQIEType);
...
std::auto_ptr<HcalQIEType> HcalTextCalibrations::produceQIEType (const HcalQIETypeRcd& rcd) {
  edm::ESHandle<HcalTopology> htopo;
  rcd.getRecord<HcalRecNumberingRecord>().get(htopo);
  const HcalTopology* topo=&(*htopo);
  return produce_impl<HcalQIEType> (topo,mInputs ["QIEType"]);
}
