...
namespace cond {
	template<>
	class ValueExtractor<HcalQIEType>: public  BaseValueExtractor<HcalQIEType> {
	public:
		typedef HcalQIEType Class;
	...
	class HcalQIETypeDataRepr: public ADataRepr
	{
	public:
		HcalQIETypeDataRepr(unsigned int total, HcalQIEType::tAllContWithNames const & allCont)
			:ADataRepr(total), allContainers(allCont){}



	protected:
		HcalQIEType::tAllContWithNames allContainers;

		void doFillIn(std::vector<TH2F> &graphData) override{
			//ITERATORS AND VALUES:
			HcalQIEType::tAllContWithNames::const_iterator iter;
			std::vector<HcalQIETyp>::const_iterator contIter;
			float value = 0.0;
	...
	template<>
	std::string PayLoadInspector<HcalQIEType>::summary() const {
		std::stringstream ss;
		unsigned int totalValues = 1;
		// get all containers with names
		HcalQIEType::tAllContWithNames allContainers = object().getAllContainers();
	...
	template<>
	std::string PayLoadInspector<HcalQIEType>::plot(std::string const & filename,
		std::string const &,
		...
		std::vector<float> const& ) const 
	{
		...
		HcalQIETypeDataRepr datarepr(numOfValues, object().getAllContainers());

		datarepr.nr = 0;
		datarepr.id = 0;
		datarepr.rootname.str("_HcalQIETyprootvalue_");
		datarepr.plotname.str("HcalQIETyp ");
...
PYTHON_WRAPPER(HcalQIEType,HcalQIEType);
